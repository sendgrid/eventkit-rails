...
class AssetsController < ApplicationController
  ...
  def authenticate
    if User.count > 0
      ...
      if cookies['auth_token'] && User.where(token: cookies['auth_token'])
      ...
      authenticate_or_request_with_http_basic('Authorized users only') do |u, p|
        ...
        if User.where(username: u).present?
          User.where(username: u).each do |user|
            next unless Password.new(user.password).is_password?(p) && (user.permissions & Permissions::VIEW == Permissions::VIEW)
