...
class ReceiverController < ApplicationController
  ...
  def header_check
    ...
    if (agent == 'SendGrid Event API') || (agent == 'SendGrid Event API Test')
      if User.count > 0
        authenticate_or_request_with_http_basic('Authorized users only') do |u, p|
          ...
          if User.where(username: u).present?
            User.where(username: u).each do |user|
              if Password.new(user.password).is_password?(p) && (user.permissions & Permissions::POST == Permissions::POST)
                valid = true
                user.issue_token
              end
            end
  ...
  def handle_post
    ...
    if Setting.where(name: 'autodelete_time').present?
      ...
      if value > 0
        ...
        Event.where(['timestamp < ?', threshold]).each(&:destroy)
      end
    end
  end
